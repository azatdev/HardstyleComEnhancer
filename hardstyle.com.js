...
const css = `
.playing {
  outline: 2px dashed ${$playingBorder};
  position: relative;
  z-index: 2;
}
.played {
...
.notdone {
  background-color: ${$notDone};
}
#player_bar a.wishlisted {
  background-color: ${$wishlisted};
}
i.wishlisted {
  color: ${$wishlisted};
  position: absolute;
...
class HardstyleComEnhancer {
  ...
  addClassToDeleteButton() {
    const targetCells = $('.text-3').toArray();
    for (const cell of targetCells) {
      const button = $(cell).children('a').first();
      button.addClass('delete-button');
      const itemId = parseInt(button.attr('onclick').match(/\d+/gi).join(''));
      button.data('itemId', itemId);
      button.removeAttr('onclick');
    }
  }
  // in the releases add a class to Add to wishlist button to hijack and enhance functionality
  addClassToWishlistButton() {
    const targetCells = $('.text-3').toArray();
    for (const cell of targetCells) {
      const button = $(cell).children('a').first();
      button.addClass('wishlist-button');
      const itemId = parseInt(button.attr('onclick').match(/\d+/gi).join(''));
      button.data('itemId', itemId);
      button.removeAttr('onclick');
    }
  }
  // ajax call to hardstyle.com servers to get track info.
  callHome(endpoint, payload) {
  ...
  async constructItem(itemId) {
    const [remotErr, remoteItem] = await handle(this.callHome('https://www.hardstyle.com/call/playlist', {track_id: itemId}));
    if (remotErr) return console.log(remotErr);
    const [err, itemRecord] = await handle(this.getRecord('track', itemId));
    if (err) return console.log(err);
    const item = {
      type: 'track',
      id: itemId,
      title: remoteItem.track_title,
      artist: remoteItem.track_artist,
      mixType: undefined,
      youtubeLink: this.constructYoutubeLink(remoteItem.track_artist, remoteItem.track_title),
  ...
  constructYoutubeLink(artist, title) {
    return `https://duckduckgo.com/?q=!ducky+youtube+${artist.replaceAll(',','').replaceAll(' ','+').replaceAll('&', '%26')}+-+${title.replaceAll(',','').replaceAll(' ','+').replaceAll('&', '%26')}`
  }
  // get id of an item from its url
  extractId(url) {
    const itemId = url.split('/').last().split('=').last();
    return parseInt(itemId.replace('.html', ''));
  }
  // extract item title, mixType, artist
  extractInfo(row) {
    let infoArray = [];
    const children = $(row).find('.text-1').first().children('a').children().toArray();
    for (const child of children) {
      infoArray.push($(child).text());
    };
    const infoObject = {
      title: infoArray.shift(),
      artist: infoArray.pop(),
  ...
  getItem(itemId) {
    return this.items.find(element => element.id === itemId);
  }
  // filter through the class items and set it to listened
  getListenedTracks() {
    return this.items.filter(element => element.listenedTo).length;
  }
  // figure out page type from url
  getPageType() {
    const pathname = window.location.pathname;
    switch(true) {
      case pathname.includes('top40'):
        this.pageType = 'top40';
        break;
      case pathname.includes('charts/album'):
        this.pageType = 'topAlbums';
        break;
      case pathname.includes('charts'):
        this.pageType = 'charts';
        break;
      case pathname.includes('albums'):
        this.pageType = 'album';
        break;
      case pathname.includes('releases'):
        this.pageType = 'releases';
        break;
      case pathname.includes('wishlist'):
        this.pageType = 'wishlist';
        break;
      case pathname.includes('tracks'):
        this.pageType = 'track';
        break;
      default:
        return;
  ...
  getRecord(itemType, itemId) {
    const itemRecord = this.db[itemType === 'album' ? 'albums' : 'tracks'].where({
      id: itemId,
    }).first();
    return itemRecord;
  }
  ...
  async initDB() {
     this.db.version(1).stores({
      tracks: "id,wishlisted",
      albums: "id,tracksListened,numberOfTracks"
    });
  }
  // highlight row
  markRow(row, itemType, itemRecord) {
    if (itemType === 'track') return $(row).addClass('played');
    if (itemRecord.tracksListened) {
      const rowClass = itemRecord.tracksListened === itemRecord.numberOfTracks ? 'played' : 'notdone';
      const rowTotals = `Listened to ${itemRecord.tracksListened} out of ${itemRecord.numberOfTracks} tracks`;
      const strong = $('<strong></strong>');
      strong.text(rowTotals);
      strong.appendTo($(row).find('.text-1').first().children('a'))
      $(row).addClass(rowClass);
    }
  }
  // go through table rows and generate data array
  async processItems() {
    if (!this.pageType) return false;
    const rows = $(".list").find("tr").toArray();
    let numberOfTracks = 0;
    for (const row of rows) {
      let itemType;
      if ($(row).find(".button.play").length) {
        itemType = 'track';
      } else if ($(row).find(".info-button").length || $(row).find(".info").length) {
        itemType = 'album';
      }
      if (!itemType) continue;
      const itemId = this.extractId($(row).find('a').first().attr('href'));
      const itemInfo = this.extractInfo(row);
      const [err, itemRecord] = await handle(this.getRecord(itemType, itemId));
      if (err) return console.log(err);
      if (itemRecord) this.markRow(row, itemType, itemRecord);
      const itemObject = {
        type: itemType,
        id: itemId,
        ...itemInfo,
        youtubeLink: this.constructYoutubeLink(itemInfo.artist, itemInfo.title),
        wishlisted: itemRecord ? itemRecord.wishlisted : false,
        listenedTo: itemRecord && itemType === 'track' ? true : false,
      };
      this.items.push(itemObject);
      $(row).attr('data-itemId', itemId);
      if (itemRecord && itemRecord.wishlisted) this.wishlistRow(itemId, itemRecord.wishlisted);
      numberOfTracks++
    }

    if (this.pageType === 'album') {
      const albumId = this.extractId(window.location.href);
      this.albumInfo = {id: albumId, numberOfTracks, tracksListened: this.getListenedTracks() };
      this.updateAlbumInfo();
    } else if (this.pageType === 'releases') {
      this.addClassToWishlistButton();
  ...
  setListened(itemId) {
    const foundItem = this.getItem(itemId);
    foundItem.listenedTo = true;
    if (this.pageType === 'album') {
      this.albumInfo['tracksListened'] = this.getListenedTracks();
      this.updateAlbumInfo();
    }
  }
  ...
  async updateAlbumInfo() {
    ...
    if (err) return console.log(record);
  }
  // update the player UI based on item
  updatePlayer(item) {
    const youtubeLink = this.constructYoutubeLink(item.artist, item.title);
    $('#player_youtube_button').attr('href', youtubeLink);
    ...
    if (item.wishlisted) {
      ...
      $("#player_wishlist_button").removeClass('wishlisted');
    }
  }
  // update item by (key, payload)
  updateRecord(itemInfo, payload) {
    const itemRecord = this.db[itemInfo.type === 'album' ? 'albums' : 'tracks'].update(itemInfo.id, payload);
    return itemRecord;
  }
  // remove useless buttons, add useful buttons
  upgradePlayer() {
    $('#jp_container_1 li:first').remove();
    $('#player_playlist_button').parents('li').remove();
    $('#player_share_button').parents('li').remove();
    const wishlistButton = $('<li class="right"><a class="player-button" id="player_wishlist_button"><i class="fa fa-star"></i></a></li>');
    $('#player_buy_button').after(wishlistButton);
    const playButtonItem = $("#player_play_button").parent('li');
    const forwardButton = $('<li class="right"><a id="player_forward_button" class="player-button player-button-round"><i class="fa fa-forward"></i></a></li>');
    playButtonItem.before(forwardButton);
    const backwardButton = $('<li class="right"><a id="player_backward_button" class="player-button player-button-round"><i class="fa fa-backward"></i></a></li>');
    playButtonItem.after(backwardButton);
    ...
    setTimeout(async () => {
      ...
      let foundItem = this.getItem(itemId) || await this.constructItem(itemId);
      this.updatePlayer(foundItem);
    }, 1000);
  }
  ...
  // if the item is wishlisted and exists in the table, mark it with an orange wishlist star
  wishlistRow(itemId, toggle) {
    const row = $('tr[data-itemid="'+ itemId +'"]');
    if (!row.length) return false;
    if (toggle) {
      const wishlistStar = $('<i class="wishlisted row-wishlist fa fa-star"></i>');
      const targetAnchor = $('tr[data-itemid="'+ itemId +'"]').find('td').first().find('a').first();
      targetAnchor.prepend(wishlistStar);
    } else {
      const row = $('tr[data-itemid="'+ itemId +'"]');
      if (this.pageType === 'wishlist') return row.remove();
      row.find('.row-wishlist').first().remove();
    }
  }
  // when the call to hardstyle.com wishlist is complete, this function will be triggered to record the change locally
  async wishlistCallComplete(toggle) {
    if (!this.wishlistItem) return false;
    const [err] = await handle(this.updateRecord({type: 'track', id: this.wishlistItem.id}, {wishlisted: toggle ? 1 : 0}));
    if (err) return console.log(err);
    this.wishlistRow(this.wishlistItem.id, toggle);
    if (this.wishlistItem.id === this.playerItem.id) {
      this.playerItem.wishlisted = toggle;
      this.updatePlayer(this.playerItem);
    }
    this.wishlistItem = undefined;
  }
  // mark the track as wishlisted or not in local db and hardstyle.com
  async wishlistToggle(item) {
    this.wishlistItem = item;
    let toggle = !item.wishlisted;
    let id = item.id; 
    let payload = {'track_id': id.toString()};
    if (!toggle) payload = {'options':{'action':'delete'}, ...payload};
    wishlist(undefined, payload);
  }
}
// start function to begin functionality
function start() {
  ...
  $('#player_wishlist_button').click(function() {
    enhancer.wishlistToggle(enhancer.playerItem);
  });

  $('#column-middle').on('click', '.wishlist-button, .delete-button', function(ev) {
    ev.preventDefault();
    const button = ev.currentTarget;
    const itemId = $(button).data('itemId');
    const item = enhancer.getItem(itemId);
    enhancer.wishlistToggle(item);
    return true;
  });
  ...
  $(document).ajaxComplete(function(ev, XMLHttpRequest, ajaxOptions) {
    ...
    switch(true) {
      case requestUrl.includes('call/wishlist'):
        const reponseText = XMLHttpRequest.responseJSON.text;
        if (reponseText === 'Added to wish list') {
